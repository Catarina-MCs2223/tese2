...
for label in set(labels):
    ...
    for i in range(len(labels)):
        if (labels[i] == label):
            axs[row_i, column_j].plot(myHouses[i].mean(), c="gray", alpha=0.4)
...
for label in set(labels):
    ...
    for i in range(len(labels)):
        if (labels[i] == label):
            cluster.append(myHouses[i])
    if len(cluster) > 0:
        plt.plot(np.average(np.vstack(cluster), axis=0), c="black")
        plt.axvline(153, color='red', linestyle='--')
        plt.axvline(302, color='red', linestyle='--')
        plt.axvline(30, color='red', linestyle='--')
        plt.text(210, 4500, 'Estação Fria', fontweight='semibold', color='red', fontdict=None)
        plt.text(65, 4500, 'Estação Quente', fontweight='semibold', color='red', fontdict=None)
        plt.text(310, 4500, 'Estação Muito Quente', fontweight='semibold', color='red', fontdict=None)
        plt.xlabel("Data")
        plt.ylabel("Usage_kW")
        plt.title("Time Series K-Means", fontweight='bold', fontsize='large')
